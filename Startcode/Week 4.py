...
def main():
    ...
    while True:
        ...
        print("4. Genereer PDF van recept")
        print("5. Exit")
        ...
        if keuze == "1":
            ...
            if not recepten:
                print("Geen recepten beschikbaar.")
                continue
            print("Kies een recept om een PDF van te maken:")
            for index, r in enumerate(recepten, start=1):
                print(f"{index}. {r._Recept__naam}")
            try:
                num = int(input("Typ het nummer van het recept: ").strip())
                if 1 <= num <= len(recepten):
                    recept = recepten[num-1]
                else:
                    print("Ongeldig nummer.")
                    continue
            except ValueError:
            ...
            pdf.cell(200, 10, txt=f"Recept: {recept._Recept__naam}", ln=True)
            pdf.cell(200, 10, txt=f"Omschrijving: {recept._Recept__omschrijving}", ln=True)
            pdf.cell(200, 10, txt=f"Voor {aantal_personen} personen", ln=True)
            pdf.cell(200, 10, txt="", ln=True)
            pdf.cell(200, 10, txt="Ingrediënten:", ln=True)
            totale_kcal = 0
            for ingredient in recept.get_ingredienten():
                if plantaardig and ingredient.plantaardig_alternatief:
                    naam = str(ingredient.plantaardig_alternatief)
                    ...
                    naam = ingredient.naam
                try:
                    hoeveelheid = float(ingredient.hoeveelheid) * aantal_personen
                    hoeveelheid = int(hoeveelheid) if hoeveelheid.is_integer() else round(hoeveelheid, 2)
                except:
                    hoeveelheid = ingredient.hoeveelheid
                try:
                    kcal = float(ingredient.kcal)
                except:
                    kcal = 0
                totale_kcal += kcal
                pdf.cell(200, 10, txt=f"- {hoeveelheid} {ingredient.eenheid} {naam} [{int(kcal)} kcal]", ln=True)
            pdf.cell(200, 10, txt="", ln=True)
            pdf.cell(200, 10, txt="Stappen:", ln=True)
            for stap in recept.get_stappen():
                pdf.multi_cell(0, 10, txt=f"- {stap.beschrijving}")
            pdf.cell(200, 10, txt="", ln=True)
            pdf.cell(200, 10, txt=f"Totale kcal per persoon: {int(totale_kcal)} kcal", ln=True)
            pdf.output(f"{recept._Recept__naam}.pdf")
            print(f"PDF opgeslagen als {recept._Recept__naam}.pdf")
